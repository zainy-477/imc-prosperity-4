...
The **Black-Scholes method** assumes that all options for the same underlying asset will naturally have the same implied volatility, however as the volatility smile below reveals, this is not the case in reality. A proposed reason is that the market is willing to pay a premium for deep in-the-money or out-of-the-money options, *to protect against market crashes and black swan events* (unpredicatable financial occurances with catastrophic impacts). Thus, the volatility smile accounts for this phenomenon, allowing us to see when an option is mispriced (the IV deviates from the fitted smile) and fill our inventory accordingly. Unfortunately, we were unable to obtain a well-fitting volatility smile (as seen in the image below) and capitalise on any mispricing, so we **only used z-score logic for this product and its options**.
