...
This writeup (authored by Zain Hirji) shares our strategies for the algorithmic and manual challenges of IMC Prosperity 4 (2026), which brought us to a global rank of 154th out of 18,000+ teams - with a total score of 485,221 XIRECs in Phase 1 and 531,123 XIRECs in Phase 2. This was the first attempt at a quantitative trading competition for all of us, and we formed a team through the Discord server created by IMC for this competition. Topics covered include mean reversion, options (including Black-Scholes and volatility smiles), news trading, game theory, and more.
...
|---|---|
| <img height="250" alt="WhatsApp Image 2026-05-13 at 22 33 33" src="https://github.com/user-attachments/assets/7020f011-6cab-476a-9c4e-360c6d51a901" /> | <img height="250" alt="image" src="https://github.com/user-attachments/assets/7e46786f-7163-4b6f-bb2a-ba303ed64177" /> |
...
For **ASH_COATED_OSMIUM**, we found that changes in the fair price had a strong negative first-order autocorrelation, as well as a mean price level of ~10,000 XIRECs across all three days of historical data. Further analysis showed that it followed the **Ornstein-Uhlenbeck process**, exhibiting mean-reverting behaviour with stochastic noise, so we derived the relevant parameters to model this product. We also noticed that when there was a *one-sided orderbook* (one side wasn't quoted), **extremely wide quotes would occasionally be filled, up to 100 XIRECs from the fair price**, providing additional alpha over the vast majority of other teams. As such, we implemented an algorithm to take advantage of the one-sided orderbook if possible, **short or long the product based on a z-score threshold**, and **market make** otherwise.
...
Our profit for this algorithmic challenge ended up at **105,692 XIRECs**, which put us into the top 100 teams. However, as we found out in Round 2, we left some profit on the table.
...
## Algorithmic Challenge - Extra Market Access
...
* **Speed was rank-based across all players.** *The lowest speed investment would receive a multiplier of 0.1 and the highest speed investment would receive a multiplier of 0.9, while everyone in between would receive a multiplier linearly scaled by rank.*

The final profit would be assigned via **PnL = (Research $\times$ Scale $\times$ Speed) - Budget_Used**.

For a given Speed investment, the remaining Research and Scale could be numerically optimised (the optimal strategy was always to use all of the allocated budget). Thus, our strategy revolved around trying to predict the multiplier one would receive for a given Speed investment. 

We assumed that a number of teams would invest nothing into Speed and that another large portion would bid up to 10% in order to try and overcut others. Furthermore, we predicted that there would be a second large wave of teams around the 30% - 40% mark who would try to reach into the upper echelons of the multiplier by outbidding all 'sensible' investments. In the Discord server and indeed from within our own team, the optimal Speed investment, as dictated by game theory, emerged to be 37%. As a result, many of us wanted to incorporate a small increase to 38% or 39% as a safety factor. However, by recognising ourselves as a part of the 'game', we ultimately decided to go beyond what we thought was necessary and invest 45% into Speed. This resulted in optimal Research and Scale investments of 14% and 41% respectively.
...
This resulted in an overall profit of **213,867 XIRECs**, compared to the optimal *217,869 XIRECs* had we invested the optimal 42% in Speed, so we were very happy with our decision. In hindsight, we should have considered that round-number bias caused large spikes in teams to choose multiples of 5% for investment in Speed, and gone slightly lower!


# Round 3

## Algorithmic Challenge - Mean Reversion and Volatility Smile
In this challenge, two underlying products were introduced - **HYDROGEL_PACK** and **VELVETFRUIT_EXTRACT** - as well as **ten call options** at distinct strike prices (4000, 4500, 5000, 5100, 5200, 5300, 5400, 5500, 6000, 6500) for the latter, similar to *VOLCANIC_ROCK* from last year.

**HYDROGEL_PACK** demonstrated mean-reverting behaviour, with changes in its price exhibiting a large negative first-order autocorrelation, but had a drifting mean rather than a steady-state one. To cater to this, we used the **Exponential Moving Average (EMA)** as an estimate of the underlying price, which gives more weight to recent prices. We also noticed that the historical spread had three peaks of concentration at 8, 12, and 16 XIRECs, leading us to identify **two main quoting behaviours** - Bot A would commonly post at ±8 XIRECs around the fair price, and Bot B occasionally posted at ±4 XIRECs around the fair price. Consequently, we came up with a two-layered system to fill our inventory, using two absolute thresholds based on mean-reversion. The *ENTRY_THRESHOLD* determined the deviation from the EMA required to fill when the spread signal was activated (i.e. Bot B was present), while the *AGGRESSIVE_THRESHOLD* determined the deviation required to cross the spread regardless of a spread signal activation. This is because a spread signal activation meant we incurred a lower loss from filling an order, so a lower entry threshold would be required to make a profit from mean reversion. To determine these constants, we once again took a page out of the Frankfurt Hedgehogs' book and performed a parameter grid sweep on our backtester, leading us to set the entry threshold at 5 XIRECs and the aggressive threshold at 45 XIRECs - as this provided the greatest profit and was in a region of stability on the heatmap.

<img width="1967" height="1331" alt="round_3_heatmap" src="https://github.com/user-attachments/assets/9a9f27da-d844-4e17-b9d4-91fd0b364f3f" />
<br><br>

The **VELVETFRUIT_EXTRACT** and its call options also exhibited mean-reverting behaviour with a drifting mean, and as such we implemented fills based on a z-score threshold using the EMA. However, we excluded **VEV_6000** and **VEV_6500** (i.e. the call options with strike prices at 6000 and 6500) as their prices were negligible and offered no room for profit. In addition to this, we used the concept of **Implied Volatility (IV)**, which is the market's forecast of how much an asset's price will move in the future, to plot a **Volatility Smile**, which graphs IV against normalised log-moneyness (where a negative value indicates the call is *in the money* and a positive value indicates the option is *out of the money*). A higher IV means the market expects greater price swings, so it is more likely that an option will finish in the money and it is priced more expensively accordingly. If you are unfamiliar with any of the concepts discussed so far, they are explained in the **Round 4 manual challenge**.

The **Black-Scholes method** assumes a constant volatility parameter, independent of the strike price; however, real option prices imply strike-dependent volatilities, producing the smile in the image below. A proposed reason is that the market is willing to pay a premium for deep in-the-money or out-of-the-money options, *to protect against market crashes and black swan events* (unpredictable financial occurrences with catastrophic impacts). Thus, the volatility smile accounts for this phenomenon, allowing us to see when an option is mispriced (the IV deviates from the fitted smile) and fill our inventory accordingly. Unfortunately, we were unable to obtain a well-fitting volatility smile (as seen in the image below) and capitalise on any mispricing, so we **only used z-score logic for this product and its options**.

<img width="2520" height="1440" alt="round_3_IV_smile" src="https://github.com/user-attachments/assets/8b98d3e3-8ede-45e1-9021-e83a7db14a7c" />

## Manual Challenge - Game Theory
This was very similar to the Round 3 manual challenge in Prosperity 3. There were a number of counterparties willing to sell the **ORNAMENTAL BIO-POD** product, each with a reserve price above which they would accept a bid. *Their reserve prices were uniformly distributed at increments of 5 between 670 and 920 XIRECs.* We would later be able to sell all inventory at 920 XIRECs. 
...
The Nash equilibrium occurred for a first bid of 751 XIRECs and a second bid of 836 XIRECs. Based on last year's results, where the average of 287 had only been slightly above the optimum of 284, we naively chose to submit a second bid of 846 XIRECs and corresponding optimal first bid of 756 XIRECs. The average second bid ended up being **859 XIRECs** and so we were heavily penalised, resulting in a PnL of **70,864 XIRECs** as opposed to the optimal *~80,000 XIRECs* - luckily, not a significant loss.
...
This algorithmic challenge retained the same products as Round 3, but now **revealed the names of bots in all trades**. The idea of this development was that some bots may have **insider information** and thus *their trades could be predictive of future price movements in a product*, offering us the chance to make favourable trades at key points. While we found this to be the case (e.g. the price would jump a certain amount for a few ticks after a specific bot traded), the **benefit of insider information did not outweigh the cost of crossing the spread** to make the necessary trades. As a result, our script did not integrate bot behaviour but retained the same trading logic as before. There was a small dispute in our team, as we had developed two improved versions of the Round 3 algorithmic script for this challenge - one that integrated market making and another that improved EMA robustness. With no time to create a joint version, we ended up going with the latter, resulting in a profit of **149,166 XIRECs**. Looking back, clear communication and team roles would have prevented the dispute from arising and likely resulted in a performance boost for our team, as market making consistently provided additional profit in our backtesting.
...
* At expiry, the CALL options are **"in the money"** if the underlying price is above their strike price (your profit per option = underlying price - strike price) or **out of the money** (i.e. worthless) if the underlying price is below their strike price, in which case the buyer loses the premium paid. Similar logic can be applied to PUT options.
...
* The **BINARY PUT Option** operates similarly to a PUT option, except it pays a fixed amount if the option is "in the money" at expiry.
...
We tackled this challenge via a two-stage process. The first was to estimate the fair value of each option contract (each of which had a contract size of 3,000) and determine whether there were any contracts that were valued above the ask or below the bid, from which we could expect to make a profit. The second was to hedge our exposure to the market, due to the large volatility that we expected from our result. Hedging is a strategy used to reduce risk and limit potential losses; essentially, it involves taking an opposite position in a related asset, so that if one investment loses money then the other gains money.

The **Black-Scholes method** is a formula used to estimate the current fair value of an option. It can be used for regular call and put options, as well as binary options. We priced the remaining chooser and knock-out put options through simulation. Based on these expected values, we chose an initial set of options for investment to maximise expected value, measured as mean profit across the 100 simulation runs, and obtained the following distributions:
...
While our approach here left much to be desired, we made four genuine observations:
...
<table>
  ...
  <tr>
    <td style="vertical-align: top; width: 50%;">
      <img src="https://github.com/user-attachments/assets/98871d10-df07-4849-8ee5-c894269ed14a" alt="image" width="700">
    </td>
    <td style="vertical-align: top; width: 50%;">
      <table>
        <thead>
...
* **Thermalite Core:** This was identical to the **VR Monocle** product from last year, except VR Monocle had just come off the back of a well-performing quarter whereas Thermalite Core was predicted to have a well-performing quarter. While VR Monocle had slightly better metrics, the combination of future growth and a useful household product (as opposed to VR Monocle, which was suggested to be damaging to the economy) led us to predict a larger growth of +35% for Thermalite Core, compared to the +22.40% experienced by VR Monocle last year. This ended up being a small blunder, as Thermalite Core experienced an almost identical growth of +22.16%.
  
* **Lava Cake:** This was similar to last year's **Quantum Coffee**, and we expected it to experience a large crash in the market; the question was how much its value would drop. On one hand, Quantum Coffee was a hype product that people had jumped on, and negative long-term effects had just been revealed. On the other hand, Lava Cake was an established and popular product with no confirmed long-term consequences. However, traces of lava were confirmed to be found within the cakes (with associated consequences for volcanic material exposure), supply in stores was to be halted, and there were lawsuits being filed against the product. We reached the conclusion that both products would experience similar drops, though Lava Cake's drop might be slightly more severe due to a complete halt in sales and legal action. As a result, compared to Quantum Coffee's drop of -66.79% last year, we predicted a return of -68% for Lava Cakes. The actual movement was -63.35%, so we were happy with our prediction.
  
* **Magma Ink:** We believed this to be a bit of a red herring for newcomers and those teams that didn't read carefully enough. Although mergers are associated with rises in valuation, this merger happened **a month ago** and so there were to be no associated gains from that. On the other hand, it could be argued that the value would fall after the **hot drop** of a combined product, as the hype around release would cause valuation to rise until the release date. However, given that the release of the product lived up to all expectations, and even slightly exceeded them, we didn't believe this to be the case. As a result, we thought there may be an insignificant rise or decline in the value of Magma Ink, and predicted no change. The actual movement ended up being +2.23%, likely because the product release was successful, but we were happy with our decision to not invest. The movement was bound to be minuscule and offer marginal returns, so there was no point in taking a risk.
  
* **Scoria Paste:** This was the easiest product for us to agree on, for three reasons. First, the supposed expert encouraging the purchase of Scoria Paste was **self-proclaimed**. Second, the broadcast took place on a YouTube equivalent platform - not very reputable for investments. Finally and most importantly, Scoria Paste is a **household object** and in common use. It is incredibly unlikely that a nobody on YouTube is going to influence the valuation of such a staple product, which is bound to have widespread supply and consistent usage. Therefore, we correctly predicted that this commodity would not experience any movement.
  
* **Ashes of the Phoenix:** We weren't too sure what to think of this, and thought the valuation could go either way. The reason for a drop is obvious - bad publicity for the company. However, the fact that a representative of the company spoke up and made a joke made us think that the popularity of the commodity might go up, especially as the video wasn't verified but contained vivid (and possibly aesthetic) imagery tied closely to the cosmetic brand's name. We might've been deeping it too much though, and in the end hedged on a small decrease with a predicted -1%. The actual movement was -3.50%, which makes sense because it was bad publicity but quickly remedied by the company.
  
* **Volcanic Incense:** Initially, we weren't convinced that there was going to be any movement at all. This commodity seemed similar to last year's **Striped Shirts**, which experienced virtually no change in value despite strong promotion. However, the article mentioned that rallies in the value of Volcanic Incense were tied to Whiff Nostralico's public appearances and commentary, and in this article he encourages the purchase of Volcanic Incense. You might be asking yourself, what kind of dumb-and-dumbers would listen to Whiff Nostralico? **Us.** We took his commentary to signify a rally and predicted an increase of +7%. This was not the case, and we should have seen it coming because its rally had already been extended this cycle, so it was overvalued with little inherent value. The actual movement was -14.57% and we took a large loss.
  
* **Sulfur Reactor:** This commodity was announced to be entering an index after its products had been considered to be industry benchmarks in a review. When a stock enters an index, funds tracking that index must purchase its shares. This brings a massive wave of capital and demand into the stock, driving its price up. While this was just an announcement, demand usually surges in order to beat the institutional demand and reap the benefits. Based on this and online data of real-world valuation jumps following announcements, we predicted a +11% return. The actual movement ended up being higher than we expected at +17.42%, perhaps because of the strength of the products (being labelled industry benchmarks and all).

<table>
  <tr>
    <td colspan="2" style="text-align: center;">
      <h2>Prosperity 4 News Trading Round</h2>
    </td>
  </tr>
    ...
    <td style="vertical-align: top; width: 50%;">
      <table>
        ...
        <tbody>
          ...
          <tr><td>Ashes of the Phoenix</td><td>-1%</td><td>-3.50%</td><td>+250</td><td>+300</td></tr>
...
Our final profit of **114,325 XIRECs** was far above the mean *35,665 XIRECs* and median *60,457 XIRECs*, though admittedly about 100,000 XIRECs came from Lava Cakes alone, which teams with access to last year's news article and commodity movements should have been able to obtain. The maximum attainable profit was *137,886 XIRECs*, although the most that any one team achieved was *131,124 XIRECs*. The majority of our deficit came from Volcanic Incense, the only product for which we predicted the direction of movement incorrectly, where we left 12,000 XIRECs on the board. All in all though, a pretty good result for the round! 

# Takeaways
All in all, this was a really enjoyable first competition. Through all the hours we invested, we learnt a lot and have grown stronger as individuals and as a team. My personal favourite moments of the competition were deducing the Python logic necessary to implement our algorithmic findings and debating about the right choice for the manual challenges - especially the news trading in Round 5. If there's any advice that I can offer to future participants, it is as follows:

* Prepare for the competition, whether this is in the days or weeks beforehand. Make sure you have all the **analytical tools and architecture ready** for when the official rounds commence. In particular, a **reliable backtester** is essential, as it allows you to rapidly test ideas (where the trial simulation on the website takes a few minutes) and across a variety of probabilistic conditions (where the trial simulation replays a deterministic simulation) to prevent overfitting.
* Read and reread the **Prosperity Wiki**. It contains all the information you need and will prevent you from being caught out.
* Make **data-driven decisions**. If something looks like it confirms your hypothesis, make sure you also have the evidence to PROVE this, else you are succumbing to confirmation bias.
